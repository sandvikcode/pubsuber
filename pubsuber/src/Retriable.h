      ...
      while (!retries_exahusted(start, backoff.RetryCount(), countPolicy, timePolicy)) {
        ...
        switch (status = (*stub.*function)(&ctx, request, &response); status.error_code()) {
          case ::grpc::StatusCode::DEADLINE_EXCEEDED:
            // Retry once again
            rpcTimeout *= 2;
            spdlog::debug("make_call failed function call '{}' with DEADLINE_EXCEEDED, new timeout is {}s", GetName<MemberFunc>(), rpcTimeout.count());
            [[fallthrough]];

          case ::grpc::StatusCode::UNAVAILABLE: {
            const auto delay = backoff.Delay();
            spdlog::debug("make_call will retry in {}ms, attempt: {}, function: '{}'", delay.count(), backoff.RetryCount() - 1, GetName<MemberFunc>());
